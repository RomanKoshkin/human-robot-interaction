...
#         print('after conv 5: ', out1.shape)
    ...
    def initHidden(self, device):
            if self.cell_type=='LSTM': # we initialize a 2-tuple of hidden states (hidden state, memory)
                return (torch.zeros(self.num_rnn_layers, 1, self.hidden_size).to(device), torch.zeros(self.num_rnn_layers, 1, self.hidden_size).to(device))
            if self.cell_type=='GRU' or self.cell_type=='RNN': # we initialize a hidden state
                return torch.zeros(self.num_rnn_layers, 1, self.hidden_size).to(device)
...
class vaeRNN(nn.Module):
    def __init__(self, cell_type, input_size, hidden_size, output_size, num_rnn_layers):
        ...
        self.fcmu = nn.Linear(input_size, input_size)
        self.fcsig = nn.Linear(input_size, input_size)
...
#                                        centered=False)
    ...
    def forward(self, visual_input, motor_input, hidden, train):
        ...
        # predict motor output based on the latent representation:
        motor_output = F.relu(self.fc3(latent.squeeze(1)))               # torch.Size([?, 16])
        
        # reconstruct video from the latent representation:
        out1 = F.relu(self.fc4(latent.squeeze(1)))                          # torch.Size([?, 64])
        out1 = out1.view(-1,1,8,8)                                      # torch.Size([?, 1, 8, 8])
        out1 = self.us1(out1)                                           # torch.Size([?, 1, 16, 16])
        out1 = F.relu(self.conv3(out1))                                 # torch.Size([?, 10, 16, 16])
        out1 = self.us2(out1)                                           # torch.Size([?, 10, 32, 32])
        out1 = F.relu(self.conv4(out1))                                 # torch.Size([?, 20, 32, 32])
        out1 = self.us3(out1)                                           # torch.Size([?, 20, 64, 64])
        out1 = torch.relu(self.conv5(out1))                             # torch.Size([?, 1, 64, 64])
